...
LEMMA DivProperty1Plt == ASSUME NEW n \in Nat, NEW x \in 0..2*n, x % 2 = 1
                 PROVE (x \div 2)+1 \in 1..n

LEMMA DivProperty2Plt == ASSUME NEW n \in Nat, NEW x \in 0..2*n, x % 2 = 0, x>0
                  PROVE (x \div 2) \in 1..n
      ...
      <2>4 ASSUME NEW p \in ProcessorSet, NEW v \in VertexSet, AddVertexTn(p, v)
           ...
           <3>1 ASSUME NEW q \in ProcessorSet, NEW t \in ProcessorSet, NEW r \in RoundSet, q \notin faulty'
                ...
                <4>2 CASE p # q \/ v.source # t \/ v.round # r
                     ...
                     <5>3 dag[q][r][t].source = t /\ dag[q][r][t].round = r
                          BY <3>1, <1>2, <5>2, <2>4 DEF IndInv6
                     <5> QED BY <5>1, <3>1, <1>2, <5>2, <2>4, <5>3 DEF IndInv6
                <4> QED BY <4>1, <4>2
...
LEMMA SpecLem == Spec => LeaderConsensus!Spec
 ...
 <1>2 ASSUME StateType, [Next]_vars
      ...
      <2>2 ASSUME NEW p \in ProcessorSet, NextRoundTn(p)
           PROVE LeaderConsensus!Next \/ UNCHANGED LeaderConsensus!vars
           BY VertexSetDefPlt, <2>2, <1>2 DEF  NextRoundTn
      <2>3 ASSUME NEW p \in ProcessorSet, NEW r \in RoundSet, NEW q \in ProcessorSet, NEW v \in VertexSet, p# q, ReceiveVertexTn(p, q, r, v)
           PROVE LeaderConsensus!Next \/ UNCHANGED LeaderConsensus!vars
           BY VertexSetDefPlt, <2>3, <1>2 DEF ReceiveVertexTn
      <2>4 ASSUME NEW p \in ProcessorSet, NEW v \in VertexSet, AddVertexTn(p, v)
           PROVE LeaderConsensus!Next \/ UNCHANGED LeaderConsensus!vars
           <3>1 CASE v.round % 2 = 1 /\ v.source = ChooseLeader((v.round \div 2)+1)
                <4>1 (v.round \div 2)+1 \in WaveSet
           ...
           <3>2 CASE v.round % 2 # 1 \/ v.source # ChooseLeader((v.round \div 2)+1)
...
\* Last modified Sat Jun 15 19:38:37 AEST 2024 by Pranav
\* Created Sat Jun 15 18:38:11 AEST 2024 by Pranav
